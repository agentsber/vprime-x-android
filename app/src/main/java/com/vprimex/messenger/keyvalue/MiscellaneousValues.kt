...
import com.vprimex.messenger.jobmanager.impl.ChangeNumberConstraintObserver
import com.vprimex.messenger.jobs.DeprecatedNotificationJob
...
class MiscellaneousValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  ...
  /**
   * Bypasses the timeout in [com.vprimex.messenger.jobs.PreKeysSyncJob] since otherwise we can hit a race.
   */
  ...
  /**
   * The last time we ran an account consistency check via [com.vprimex.messenger.jobs.AccountConsistencyWorkerJob]
   */
  ...
  /**
   * The last time we prompted the user regarding a [com.vprimex.messenger.util.ConnectivityWarning].
   */
   ...
   * [com.vprimex.messenger.service.webrtc.CallingAssets.CURRENT_VERSION] to determine
